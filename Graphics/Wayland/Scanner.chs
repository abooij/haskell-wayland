...
-- Remember that we should re-export this symbol so that it is readable
export :: String -> ProcessWithExports ()
export name = tell [name]


-- TODO everything should be exported
  ...
  liftM concat $ sequence $ map registryBindInterface globalInterfaces
    where
      ...
      registryBindInterface iface = do
        ...
        exposureDec <- lift $ [d|$(varP $ mkName exposeName) = \ reg name strname version -> withCString strname $ \cstr -> $(varE internalCName) reg 0 $(varE $ interfaceCInterfaceName pname iname) (fromIntegral (name::Word)) cstr (fromIntegral (version::Word)) nullPtr |]
        export exposeName
...
generateListener sp iface sc =
  ...
  let -- declare a Listener or Interface type for this interface
      ...
      foreignDec = case sc of
                     ...
                     Server -> forImpD cCall unsafe "wl_resource_set_implementation" foreignName [t|$(conT $ mkName $ interfaceTypeName pname iname) -> (Ptr $(conT $ typeName))  -> (Ptr ()) -> FunPtr ($(conT $ mkName $ interfaceTypeName pname iname) -> IO ()) -> IO ()|]
                     -- int wl_proxy_add_listener(struct wl_proxy *proxy,
                     --    void (**implementation)(void), void *data);
                     Client -> forImpD cCall unsafe "wl_proxy_add_listener" foreignName [t|$(conT $ mkName $ interfaceTypeName pname iname) -> (Ptr $(conT $ typeName))  -> (Ptr ()) -> IO CInt|]
      apiDec :: Q [Dec]
      apiDec = [d|$(varP $ mkName $ requestHaskName (protocolName sp) iname "set_listener") = \ iface listener ->
                   do
  ...
  in do
    ...
    last <- lift $ apiDec
    export $ requestHaskName (protocolName sp) iname "set_listener"
