     ...
     generateMessage idx msg = -- list idx used for wl_proxy_marshal arguments
       ...
       in case sc of
            Server -> do
              ...
              cdec <- forImpD cCall unsafe "wl_resource_post_event" internalCName [t|$(conT $ interfaceTypeName (protocolName ps) (interfaceName iface)) -> {#type uint32_t#} -> $(genMessageCType Nothing (messageArguments msg)) |]
            ...
            Client -> do
              ...
              cdec <- case numNewIds of
                        ...
                        1 -> forImpD cCall unsafe "wl_proxy_marshal_constructor" internalCName [t|$(conT $ interfaceTypeName (protocolName ps) (interfaceName iface)) -> {#type uint32_t#} -> Ptr CInterface -> $(genMessageCType (Just returnType) (messageArguments msg)) |]
...
applyAtPosition' fun arg 0   = do
  ...
  return ([], funexp)
applyAtPosition' fun arg pos = do
  newVar <- newName "somesecretnameyoushouldntmesswith___"
  ...
  resourceCreators <-
    case sc of
      ...
      Server -> liftM concat $ sequence $
        ...
        --   in (liftM Left) forImpD cCall unsafe "wl_resource_create" internalCName [t|Util.Client -> CInterface -> CInt -> {#type uint32_t#} |]
        --        : (liftM . map Right) [d|$(varP $ interfaceResourceCreator pname iname) = \ client id ->
        --                    $(varE internalCName) client $(varE $ interfaceCInterfaceName pname iname) $(litE $ IntegerL $ interfaceVersion iface) |]
        --   ) (protocolInterfaces sp)
        map (\ iface -> do
          let iname = interfaceName iface
              internalCName = mkName $ pname ++ "_" ++ iname ++ "_c_resource_create"
          foreignDec <- forImpD cCall unsafe "wl_resource_create" internalCName [t|Util.Client -> Ptr CInterface -> CInt -> {#type uint32_t#} -> IO $(conT $ interfaceTypeName pname iname) |]
	    ...
	    poke ptr record = $(doE $ ( zipWith (\ idx msg ->
                noBindS [e|do
                  let haskFun = $(return $ VarE $ mkMessageName msg) record
                      unmarshaller fun = \x -> $(let (pats, funexp) = argTypeUnmarshaller (messageArguments msg) ([e|fun x|]) -- FIXME server/client
                                                 in LamE pats <$> funexp)
